...
#define DOWN_MASK (0x2000)
#define UP_MASK (0x4000)
...
QueueHandle_t bug_command_data_q;
...
void task_capsense(void* param)
{
    ...
    Cy_CapSense_InitializeWidgetGestures(CY_CAPSENSE_LINEARSLIDER0_WDGT_ID, &cy_capsense_context);

    configPRINTF( ("CapSense initiated.\n\r"));
...
static void process_touch(void)
{
    ...
    uint32_t detected_gesture = 0;
    uint16_t slider_pos = 0;
    uint8_t slider_touched = 0;

    cy_stc_capsense_touch_t *slider_touch;
    traceString slider_channel = xTraceRegisterString("CapSlider value");
    ...
    static volatile bool left_detected = false;
    static volatile bool right_detected = false;

    /* Variables used for storing command and data for LED Task */
    led_command_data_t led_cmd_data;
    bool send_led_command = false;
    /* Variables to send information to bug task */
    bug_command_data_t bug_cmd_data;
    bool send_bug_command = false;
    ...
    /* Detect LEFT swipe on the slider */
    detected_gesture = Cy_CapSense_DecodeWidgetGestures(CY_CAPSENSE_LINEARSLIDER0_WDGT_ID, &cy_capsense_context);
    if (detected_gesture != 0)
    {
        if(detected_gesture & DOWN_MASK)
        {
            is_down = true;
            /*reset right/left detected here AFTER checking the slider, making any sliding happening AFTER this be what counts*/
            left_detected = false;
            right_detected = false;
        }
        else if (detected_gesture & UP_MASK)
        {
            if(is_down)
            {
                is_down = false;
                ...
                {
                    send_bug_command = true;
                    bug_cmd_data.command = SWIPE_LEFT;
                }
            } else {
                printf("up detected when not down\n");
            }
    ...
    if(send_bug_command)
    {
        xQueueSendToBack(bug_command_data_q, &bug_cmd_data, 0u);
    }
